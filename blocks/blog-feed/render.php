<?php
/**
 * Blog feed block — featured post + 2x2 grid + Zing tip.
 * TODO ACF fields: zing_tip_text (text), section_eyebrow (text), section_title (text)
 */
...
// Map category slug → badge styles
$cat_styles = [
...
function zing_get_post_cat_style( $post_id, $styles, $default ) {
	...
	$slug  = $cats[0]->slug;
	$name  = $cats[0]->name;
	return [ 'style' => $styles[ $slug ] ?? $default, 'name' => $name ];
}
?>
...
<section class="blog-feed"<?php echo $anchor; ?>>
	<div class="blog-feed__inner">
		...
		<p class="blog-feed__eyebrow"><?php esc_html_e( 'This week', 'everyday-zing-theme' ); ?></p>
		<h2 class="blog-feed__title"><?php esc_html_e( 'Fresh from the blog', 'everyday-zing-theme' ); ?></h2>
		...
		<?php if ( $query->have_posts() ) : ?>
			...
			while ( $query->have_posts() ) :
				$query->the_post();
				$post_id  = get_the_ID();
				$cat_data = zing_get_post_cat_style( $post_id, $cat_styles, $default_style );
			...
			?>
				...
				<article class="blog-feed__featured">
					...
					<div class="blog-feed__feat-body">
						<h3 class="blog-feed__feat-title">
							<a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
						</h3>
						<p class="blog-feed__feat-desc"><?php echo wp_trim_words( get_the_excerpt(), 20 ); ?></p>
						<div class="blog-feed__feat-meta">
							<div class="blog-feed__avatar">CJ</div>
							<span class="blog-feed__meta-txt">
								<?php the_author(); ?> &middot; <?php echo get_the_date(); ?> &middot; <?php echo esc_html( ceil( str_word_count( get_the_content() ) / 200 ) ); ?> min read
							</span>
						...
						<article class="blog-feed__card">
							...
							<div class="blog-feed__card-body">
								...
								<h3 class="blog-feed__card-title">
									<a href="<?php echo esc_url( $gp['link'] ); ?>"><?php echo esc_html( $gp['title'] ); ?></a>
								</h3>
							</div>
						</article>
					<?php endforeach; ?>
		...
		<!-- Zing Tip — TODO: replace with ACF field get_field('zing_tip_text') -->
		<div class="blog-feed__zing-tip">
			<p class="blog-feed__zing-label"><?php esc_html_e( 'Zing tip of the week', 'everyday-zing-theme' ); ?></p>
			<p class="blog-feed__zing-text">Pack snacks. Always. Your wallet (and your mood) will thank you mid-flight.</p>
		</div>
